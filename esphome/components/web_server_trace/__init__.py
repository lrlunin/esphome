...
web_server_ns = cg.esphome_ns.namespace("web_server_trace")

WebServer = web_server_ns.class_("WebServer", cg.Component, cg.Controller)
HistoryTrace = web_server_ns.class_("HistoryTrace")
...
WEBSERVER_SORTING_SCHEMA = cv.Schema(
    {
        cv.Optional(CONF_WEB_SERVER): cv.Schema(
            {
                cv.OnlyWith(CONF_WEB_SERVER_ID, "web_server_trace"): cv.use_id(WebServer),
                cv.Optional(CONF_SORTING_WEIGHT): cv.All(
                    cv.requires_component("web_server_trace"),
                    cv.float_,
                ),
...
HISTORY_TRACE_SCHEMA = cv.Schema(
    {
        cv.GenerateID(): cv.declare_id(HistoryTrace),
    ...
    cv.Schema(
        {
            ...
            cv.Optional(CONF_TRACES): cv.ensure_list(HISTORY_TRACE_SCHEMA),
        }
...
async def to_code(config):
    ...
    cg.add_define("USE_WEBSERVER_TRACE")
    cg.add_define("USE_WEBSERVER_TRACE_PORT", config[CONF_PORT])
    cg.add_define("USE_WEBSERVER_TRACE_VERSION", version)
    if version >= 2:
    ...
    if config[CONF_ENABLE_PRIVATE_NETWORK_ACCESS]:
        cg.add_define("USE_WEBSERVER_TRACE_PRIVATE_NETWORK_ACCESS")
    if CONF_AUTH in config:
        cg.add(paren.set_auth_username(config[CONF_AUTH][CONF_USERNAME]))
        cg.add(paren.set_auth_password(config[CONF_AUTH][CONF_PASSWORD]))
    if CONF_CSS_INCLUDE in config:
        cg.add_define("USE_WEBSERVER_TRACE_CSS_INCLUDE")
        path = CORE.relative_config_path(config[CONF_CSS_INCLUDE])
        with open(file=path, encoding="utf-8") as css_file:
            add_resource_as_progmem("CSS_INCLUDE", css_file.read())
    if CONF_JS_INCLUDE in config:
        cg.add_define("USE_WEBSERVER_TRACE_JS_INCLUDE")
    ...
    if CONF_LOCAL in config and config[CONF_LOCAL]:
        cg.add_define("USE_WEBSERVER_TRACE_LOCAL")
